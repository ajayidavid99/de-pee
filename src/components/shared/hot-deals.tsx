/* ==========================================================================\
   HOT DEALS & TENDER OFFERS COMPONENT
   ========================================================================== */
...
export function HotDeals({ products }: { products: DBProduct[] }) {
  // Take up to 4 discounted or fast-moving deal items
  ...
  return (
    <section className="w-full bg-linear-to-b from-background via-amber-500/5 to-background py-10 border-t border-border/60">
      <div className="mx-auto max-w-7xl px-4 lg:px-6">
        ...
        {/* Responsive Grid matching Category style: 2x2 Mobile | 2x2 Tablet | 4x1 Desktop */}
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          {dealProducts.map((item) => (
            <Card
              key={item.id}
              hover
              className="relative overflow-hidden rounded-xl border border-amber-500/30 bg-card/60 transition-all duration-300 group hover:border-amber-500/60 flex flex-col justify-between h-52 sm:h-56 p-3"
            >
              {/* Badge Overlay */}
              ...
              <div className="pt-2 border-t border-border/40">
                <Button size="sm" asChild className="w-full text-[10px] h-6 font-bold rounded-md bg-amber-500 text-slate-950 hover:bg-amber-400">
